...
| ----------------------- | ----------------------------------------------------- |
| `add()` vs `offer()`    | `add()` throws on failure, `offer()` returns `false`. |
| `remove()` vs `poll()`  | `remove()` throws if empty, `poll()` returns `null`.  |
| `element()` vs `peek()` | `element()` throws if empty, `peek()` returns `null`. |
